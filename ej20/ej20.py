...
async def primera(n: int) -> str:
    ...
    await asyncio.sleep(i)#duerme la tarea en segundo plano hasta que pasen los segundos, y el even loop toma el control
    result = f"result{n}-A"
...
async def chain(n: int) -> None:
    start = time.perf_counter()#inicia un temporizador
    print("Lanzando primera")
    prim = await primera(n)#queda durmiendo y corre la primera
...
async def main(*args):
    await asyncio.gather(*(chain(n) for n in args))#crea el even loop, frabrica un generador de llamadas ecorrutinas y las mete dentro del even loop
...
if __name__ == "__main__":
    ...
    print(args)
    start = time.perf_counter()#empieza a contar el tiempo con un contador
    asyncio.run(main(*args))#lansa el main con asincronismo 
    end = time.perf_counter() - start
